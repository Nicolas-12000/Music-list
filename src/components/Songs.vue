...
export default defineComponent({
  ...
  setup() {
    ...
    const junmpToSong = (index: number) => {
      const SongArray = playlist.value.toArray();
      if (index <0 || index >= SongArray.length) {
        console.error(`Índice ${index} fuera de rango. La lista tiene ${SongArray.length} elementos.`);
        return;
      }
